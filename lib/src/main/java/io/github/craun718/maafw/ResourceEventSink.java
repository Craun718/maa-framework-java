...
public class ResourceEventSink extends EventSink {
    ...
    public void onResourceLoading(
            Resource resource, MaaDef.NotificationType notificationType, ResourceLoadingDetail detail) {}
        ...
        if (message != null && message.startsWith("Resource.Loading")) {
            onResourceLoading(
                    ...
                    new ResourceLoadingDetail(
                            MaaResultParsers.longValue(details.get("res_id")) == null
                                    ? 0L
                                    : MaaResultParsers.longValue(details.get("res_id")),
                            MaaResultParsers.string(details.get("path")),
                            details.getOrDefault("type", "Bundle").toString(),
